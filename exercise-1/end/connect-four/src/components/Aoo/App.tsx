...
export default class App extends React.PureComponent<Props, State> {
  ...
  handleEmptyTileClick = (tileId: string) => {
    const { chipsPositions, playerTurn } = this.state;

    // Find the last empty tile in the column
  ...
  renderBoard() {
    const { columns, rows } = this.props;
    const { chipsPositions } = this.state;
    return <Board columns={columns} rows={rows} chipsPositions={chipsPositions} onEmptyTileClick={this.handleEmptyTileClick} />;
  }
